...
bool FPicoEyeTracker::GetEyeTrackerStereoGazeData(FEyeTrackerStereoGazeData& OutGazeData) const
{
	...
	//if (GetDefault<UPicoMobileSettings>()->EnableEyeTracking)
	//{
...
bool FPicoEyeTracker::OpenEyeTracking()
{
	...
	if (GetDefault<UPicoMobileSettings>()->EnableEyeTracking)
	{
		if (PicoImport::PvrGetTrackingMode() & 4)
		{
			...
			{
				UE_LOG(PicoEyeTracker, Log, TEXT("Pvr_UE Enable Eye Tracking Succeed!"));
				bEyeTrackingRun = true;
				retValue = true;
			}
			else
			{
				UE_LOG(PicoEyeTracker, Log, TEXT("Pvr_UE Enable Eye Tracking Failed!"));
			}
		}
		else
		{
			UE_LOG(PicoEyeTracker, Log, TEXT("Pvr_UE Devices is not support EyeTracking!"));
		}	
	}
	else
	{
		UE_LOG(PicoEyeTracker, Log, TEXT("Pvr_UE Do not Open EyeTracking!"));
	}
#endif
	return retValue;
}
...
bool FPicoEyeTracker::GetEyeTrackingGazeRay(FEyeTrackingGazeRay &EyeTrackingGazeRay)const
{
	...
	if (EyeTrackingGazeRay.IsValid)
	{
		...
		{
			HeadLocation = PicoSDK::Sensor::GCurrentPosition;
			HeadRotation = PicoSDK::Sensor::GCurrentOrientation.Rotator();
		}
...
bool FPicoEyeTracker::GetEyeDirectionToFFR(FVector &OutDirection) const
{
	if (bEyeTrackingRun)
	{
		...
		if (bEyeTrackingRun)
		{
			...
			FVector EyeDirections[3] = { FVector(TempTrackingData.leftEyeGazeVector.X,TempTrackingData.leftEyeGazeVector.Y,-TempTrackingData.leftEyeGazeVector.Z)
										,FVector(TempTrackingData.rightEyeGazeVector.X,TempTrackingData.rightEyeGazeVector.Y,-TempTrackingData.rightEyeGazeVector.Z)
										,FVector(TempTrackingData.combinedEyeGazeVector.X,TempTrackingData.combinedEyeGazeVector.Y,-TempTrackingData.combinedEyeGazeVector.Z) };
			EyeDirections[0] = EyeDirections[1] = EyeDirections[2];
