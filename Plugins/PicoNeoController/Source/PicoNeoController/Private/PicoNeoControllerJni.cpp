...
FPicoNeoControllerJni::FPicoNeoControllerJni()
{
	...
	id_GetControllerEvent = FJavaWrapper::FindMethod(Env, FJavaWrapper::GameActivityClassID, "getCVControllerButtonEvent", "(I)[I", false);
}
...
void FPicoNeoControllerJni::GetControllerButtonEvent(int controller, int* OutButton)
{
	JNIEnv* Env = FAndroidApplication::GetJavaEnv();
	jintArray jiArray = (jintArray)FJavaWrapper::CallObjectMethod(Env, FJavaWrapper::GameActivityThis, id_GetControllerEvent, controller);
	jint *jip = Env->GetIntArrayElements(jiArray, JNI_FALSE);
	for (int i = 0; i < 67; i++)
	{
		OutButton[i] = jip[i];
