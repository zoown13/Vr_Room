...
#define LOCTEXT_NAMESPACE "PicoMobile"
...
FPicoMobileController::FPicoMobileController(const TSharedRef<FGenericApplicationMessageHandler>& InMessageHandler)
	...
	LastOrientation(0.f, 0.f, 0.f),
	LastPosition(0.f, 0.f, 0.f)
{	
#if PLATFORM_ANDROID
...
void FPicoMobileController::AddKeysToEngine()
{
	...
	EKeys::AddKey(FKeyDetails(FPicoMobileControllerKeys::Pico_Handle_TouchX, LOCTEXT("Pico_Handle_TouchX", "Pico Handle TouchX"), FKeyDetails::GamepadKey | FKeyDetails::FloatAxis));
	EKeys::AddKey(FKeyDetails(FPicoMobileControllerKeys::Pico_Handle_TouchY, LOCTEXT("Pico_Handle_TouchY", "Pico Handle TouchY"), FKeyDetails::GamepadKey | FKeyDetails::FloatAxis));
...
void FPicoMobileController::SetKeyMapping()
{
	...
	//{
	//	LastButtonStates[buttonIndex] = false;
	//}
}
...
inline void FPicoMobileController::ProcessControllerButtons()
{
	...
	/*	for (int32 buttonIndex = 6; buttonIndex < 14; buttonIndex++)
		{
			if (CurrentButtonDownStates[buttonIndex] != LastButtonStates[buttonIndex - 6])
			{
				if (CurrentButtonDownStates[buttonIndex])
				{
					MessageHandler->OnControllerButtonPressed(Buttons[buttonIndex], 0, false);
				...
				LastButtonStates[buttonIndex - 6] = CurrentButtonDownStates[buttonIndex];
			}
		}*/
#endif
}
...
int32 FPicoMobileController::GetTriggerKeyEvent()
{
	int32 triggerKeyEvent = 0;
#if PLATFORM_ANDROID
	JNIEnv* Env = FAndroidApplication::GetJavaEnv();
	jmethodID id_apl_GetTriggerKeyEvent = FJavaWrapper::FindMethod(Env, FJavaWrapper::GameActivityClassID, "apl_GetTriggerKeyEvent", "()I", false);
	triggerKeyEvent = StaticCast<int32>(FJavaWrapper::CallIntMethod(Env, FJavaWrapper::GameActivityThis, id_apl_GetTriggerKeyEvent));
#endif
	return triggerKeyEvent;
}
...
void FPicoMobileController::UpdateButtonEvent()
{
		...
		{
			Hand.TouchValue.Y = (((float)TempButton[0]) / 255 - 0.5f) * 2;
			Hand.TouchValue.X = (((float)TempButton[5]) / 255 - 0.5f) * 2;
		}
...
{
#if PLATFORM_ANDROID
	if ((int32)pvrControllerState->GetHBArmJoint() == 0)
	{
		SetUseWrist();
	}
	else if ((int32)pvrControllerState->GetHBArmJoint() == 1)
	{
		SetUseElbow();
	}
	else if ((int32)pvrControllerState->GetHBArmJoint() == 2)
	{
		SetUseShoulder();
	}
	else if ((int32)pvrControllerState->GetHBArmJoint() == 3)
	{
		SetUseController();
	}
#endif
